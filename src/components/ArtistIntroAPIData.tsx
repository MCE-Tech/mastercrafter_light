...
export default function ArtistIntroAPIData({ artist, artistDetails }: Readonly<ArtistIntroProps>) {
    console.log("[ArtistIntroAPIData] artist=", artist ? artist.slug : null, "artistDetails=", artistDetails ? artistDetails : null);
    return (
        <div className="flex flex-col items-center md:items-start h-full px-4 md:px-0 py-6 md:py-0">
            ...
            <div className="artist-profile-tags m-auto flex flex-wrap gap-2 justify-center md:justify-start mb-4 md:mb-2">
                {
                Object.entries(artistDetails?.defaultTags || []).map(([ , tagValue,], index) => (
                    tagValue!=null?(
                        <span
                            key={index}
